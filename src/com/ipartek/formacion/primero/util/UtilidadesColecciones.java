...
public class UtilidadesColecciones {
	 ...
	 * @return array tipo Integer[]
	 */
	public static Float[] toObjectFloat(float[] intArray) {

		Float[] result = new Float[intArray.length];
		for (int i = 0; i < intArray.length; i++) {
			result[i] = Float.valueOf(intArray[i]);
		}
		return result;
	}
	 ...
	 * @return Array tipo int[]
	 */
	public static int[] toPrimitiveFloat(Float[] floatArray) {

		int[] result = new int[floatArray.length];
		for (int i = 0; i < floatArray.length; i++) {
			result[i] = floatArray[i].intValue();
		}
		return result;
	}
	...
	/**
	 * Ordena un array de tipo float[] <br>
	 * Utilizando conversión del array a tipo objeto Integer[] y devolviéndolo
	 * al tipo original
	 *
	 * @param myArray
	 ...
	 * @return El array ordenado
	 */
	public static float[] sortedArrayFloat(float[] myArray, boolean descent) {
		final Float[] sorted = UtilidadesColecciones.toObjectFloat(myArray);
		...
		System.arraycopy(UtilidadesColecciones.toPrimitiveFloat(sorted), 0,
				myArray, 0, sorted.length);
		return myArray;
	}
