...
public class Utilidades {
	...
	//Sobrecargamos el metodo para que pueda ser llamado sin parametros y nos pinte una linea por defecto
	 ...
	 * @param c <code>char</code>: el caracter a evaluar
	 * @return <code>true</code> si es vocal, <code>false</code> si es consonante 
	 */
	 ...
	 * @param dni <code>String</code>: el dni a evaluar
	 * @return esCorrecto <code>Boolean</code>: indica si el dni es correcto o no
	 */
	...
	public static boolean comprobarDni(String dni){
		...
		if(dni.length() == 9){//Comprobamos que tenga la longitud apropiada
			...
			if(numeroDni >= 00000000 && numeroDni <= 99999999){
				...
				//Recorremos el abecedario en busca de la letra del dni que nos han introducido
				...
				if(flag){
					...
					System.out.println("El caracter que completa el dni no es correcto");
				}
			}else{
			...
			System.out.println("La longitud del dni no es correcta, por favor introduzca un dni correcto");
		}
		return esCorrecto;
	}
	...
	public static boolean comprobarTelefono(String telefono){
		...
		//Pattern patron_telefono = Pattern.compile("^\+?\d{1,3}?[- .]?\(?(?:\d{2,3})\)?[- .]?\d\d\d[- .]?\d\d\d\d$");
